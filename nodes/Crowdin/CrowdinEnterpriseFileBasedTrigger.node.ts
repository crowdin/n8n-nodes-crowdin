...
// Organization-level events that don't require a project (Enterprise only)
const ORGANIZATION_EVENTS = ['project.created', 'project.deleted', 'group.created', 'group.deleted'];

// File-based project events from OpenAPI specification
const FILE_BASED_EVENTS = [
	// Organization-level events (Enterprise only)
	{
	...
	// File events (specific to file-based projects)
	{
		name: 'File added',
		value: 'file.added',
...
export class CrowdinEnterpriseFileBasedTrigger implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Crowdin Enterprise (File-Based) Trigger',
		name: 'crowdinEnterpriseFileBasedTrigger',
		icon: { light: 'file:../../icons/crowdin.svg', dark: 'file:../../icons/crowdin.dark.svg' },
		group: ['trigger'],
		version: 1,
		subtitle: '={{$parameter["event"]}}',
		description: 'Starts the workflow when Crowdin Enterprise file-based project events occur',
		defaults: {
			name: 'Crowdin Enterprise Trigger (File-Based)',
		},
		inputs: [],
	...
	webhookMethods = {
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				const isOrganizationEvent = ORGANIZATION_EVENTS.includes(event);
						...
						for (const webhook of webhooks) {
							if (webhook.url === webhookUrl) {
								...
								if (events && events.includes(event)) {
									...
									webhookData.isOrganizationWebhook = true;
									return true;
								}
							}
						}
						...
						for (const webhook of webhooks) {
							if (webhook.url === webhookUrl) {
								...
								if (events && events.includes(event)) {
									const webhookData = this.getWorkflowStaticData('node');
									webhookData.webhookId = webhook.id;
									webhookData.isOrganizationWebhook = false;
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				try {
					...
					throw new NodeApiError(this.getNode(), { error: message } as JsonObject, {
						message: `Failed to create Crowdin Enterprise webhook: ${message}`,
					});
				}
			},
			...
			async delete(this: IHookFunctions): Promise<boolean> {
				...
				const isOrganizationWebhook = webhookData.isOrganizationWebhook as boolean;
				...
				try {
					...
					delete webhookData.isOrganizationWebhook;
