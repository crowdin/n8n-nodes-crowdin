...
export const stringAssetTranslationsProperties: INodeProperties[] = [
	{
		...
		options: [
			{
				...
				description: '**Required scopes:** `project.translation` (Read only).\n\n__Note:__ Either `translationId` OR `correctionId` OR `fileId` OR `labelIds` OR `excludeLabelIds` with `languageId` OR `stringId` with `languageId` are required',
				routing: {
					request: {
						method: 'GET',
			...
			{
				name: 'Remove String/Asset Approvals',
				value: 'api.projects.approvals.deleteMany',
				action: 'Remove String/Asset Approvals',
				description: '**Required scopes:** `project.translation` (Read and Write).\n\nRemove all approvals for a string or asset. Use the oneOf pattern:\n- **String type**: Use `stringId`\n- **Asset type**: Use `fileId`',
				routing: {
					request: {
						method: 'DELETE',
						url: '=/projects/{{$parameter["projectId"]}}/approvals'
			...
			{
				name: 'Delete String/Asset Translations',
				value: 'api.projects.translations.deleteMany',
				action: 'Delete String/Asset Translations',
				description: '**Required scopes:** `project.translation` (Read and Write).\n\nDelete translations for a string or an asset. Use the oneOf pattern:\n - **String type**: Use `stringId`\n - **Asset type**: Use `fileId`',
				routing: {
					request: {
						method: 'DELETE',
						url: '=/projects/{{$parameter["projectId"]}}/translations'
	...
	},
	{
		...
		name: 'translationId',
		description: 'Translation Identifier. Get via [List String Translations](#operation/api.projects.translations.getMany) <br> **Note:** If specified, `fileId`, `stringId` and `languageId` are ignored. Cannot be used with `correctionId`',
		default: 0,
		type: 'number',
		routing: {
			send: {
	...
	},
	{
		...
		name: 'correctionId',
		description: 'Correction Identifier. Get via [List Corrections](#operation/api.projects.corrections.getMany) <br> **Note:** Cannot be used with `translationId`, `languageId`, `stringId` or `fileId`. Available only for projects with advanced workflow',
		default: 0,
		type: 'number',
		routing: {
			send: {
				type: 'query',
				property: 'correctionId',
				value: '={{ $value !== 0 ? $value : undefined }}',
				propertyInDotNotation: false
			}
		},
				...
				values: [
					{
						...
						type: 'number',
						default: undefined,
						description: 'Correction Identifier. Get via [List Corrections](#operation/api.projects.corrections.getMany)',
						required: true,
						placeholder: '35'
					}
				]
	...
	},
	{
		displayName: 'File Id',
		name: 'fileId',
		description: 'Filter translations by `fileId`. Get via [List Files](#operation/api.projects.files.getMany)\n\n__Note:__ Can\'t be used with `branchId` or `directoryId` in the same request',
		default: '',
		type: 'options',
		routing: {
			send: {
	...
	},
	{
		displayName: 'Croql',
		name: 'croql',
		description: 'Filter translations by [CroQL](https://developer.crowdin.com/croql/)\n\n__Note:__ Can\'t be used with `stringIds`, `labelIds`, `fileId`, `passedWorkflow` or `minApprovalsCount` in same request',
		default: '',
		type: 'string',
		routing: {
			send: {
	...
	},
	{
		...
		options: [
			{
				displayName: 'Translation Create Form',
				name: '_translationCreateForm',
				values: [
					{
						displayName: 'String Id',
						name: 'stringId',
			...
			{
				displayName: 'Asset Translation Create Form',
				name: '_assetTranslationCreateForm',
				values: [
					{
						displayName: 'File Id',
						name: 'fileId',
					...
					},
					{
						...
						type: 'options',
						default: '',
						description: 'Language Identifier. Get via [List Languages](#operation/api.projects.languages.getMany)',
						required: true,
						typeOptions: {
							loadOptionsMethod: 'getLanguages'
						}
